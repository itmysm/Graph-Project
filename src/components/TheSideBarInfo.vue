<template>
  <div class="w-[400px] border border-accent ltr:border-r rtl:border-l bg-base-1 h-screen transition-all duration-500 relative" :class="sidebarInfoIsOpen ? 'w-96 px-5' : 'w-[1px] px-0'">
    <a class="w-5 h-10 flex justify-center border-none items-center rounded-l-lg bg-primary absolute ltr:left-[-22px] mt-14 cursor-pointer select-none" @click="sidebar(null)">
      <i class="material-symbols-rounded text-white transition-all rtl:rotate-180 text-[20px] text-2xl" :class="sidebarInfoIsOpen ? 'rotate-180' : 'rotate-0'">arrow_left</i>
    </a>
...
function sidebar () {
  sidebarInfoIsOpen.value = !sidebarInfoIsOpen.value
  emit('sideBarInfoUpdated', sidebarInfoIsOpen.value)
}

watch(() => props.sideBar, (newVal) => {
  if (window.innerWidth > 1400) sidebarInfoIsOpen.value = !newVal
})
