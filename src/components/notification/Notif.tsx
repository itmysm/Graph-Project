...
export default function Notif({ notif }: { notif: Notification }) {
  ...
  return (
    <div
      className={`${
        `light flex justify-between items-center min-w-[300px] bg-` + alertTypes[notif.type].bg
      } px-3 py-5 rounded-xl transition-all animate__animated ${onVisible ? "animate__fadeInDown" : "animate__fadeOutUp"}`}
    >
