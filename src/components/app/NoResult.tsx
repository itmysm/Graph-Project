import { RootObject, Page } from "@/types/locales";
...
export default function NoResult({ i18n, onOpenUploadDialog }: Props) {
  ...
  return (
    <div className="flex flex-col justify-center items-center">
      <p className="text-lg tracking-wide text-contrast/70 font-semibold mt-10 mb-4">{i18n.app.noUploadedFile}</p>
      ...
      <div className="w-full flex justify-center items-center">
        ...
        <p className="text-sm text-contrast mx-3 tracking-wide">{i18n.app.separator}</p>
        <Button className="text-contrast font-semibold bg-secondary hover:bg-secondary/90 rounded-lg">
          {i18n.app.useTemplate}
        </Button>
      </div>
    </div>
  );
